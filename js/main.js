...
window.addEventListener('load', () => {
    ...
    alarm = new Audio('/assets/sound/alarm.mp3')
})
...
function setTimer() {
    clock.innerHTML = `<p class='number'>${hours > 9 ? hours : ('0' + hours)}</p><span>hrs</span><p class='number'>${minutes > 9 ? minutes : ('0' + minutes)}</p><span>min</span><p class='number'>${seconds > 9 ? seconds : ('0' + seconds)}</p><span>sec</span>`;
